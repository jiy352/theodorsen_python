...
def geom_setup(P, PC, Swimmer, solid=None, FSI=None, PyFEA=None):
    ...
    SwiL     = [None for x in range(P['N_SWIMMERS'])]
    GeoL     = [None for x in range(P['N_SWIMMERS'])]
    MotL     = [None for x in range(P['N_SWIMMERS'])]
    Swimmers = [None for x in range(P['N_SWIMMERS'])]
    SolidL   = [None for x in range(P['N_SWIMMERS'])]
    FSIL     = [None for x in range(P['N_SWIMMERS'])]
    PyFEAL   = [None for x in range(P['N_SWIMMERS'])]

    for i in range(P['N_SWIMMERS']):
...
def multi_kinematics(P, PHI=0., scale=None, rate=50):
    ...
    # Sine Wave Input
    sineFrac      = [scale[0] * np.sin(2 * np.pi * P['F'] * P['T'][i]                + PHI) for i in range(P['COUNTER'])]
    sineFracMinus = [scale[0] * np.sin(2 * np.pi * P['F'] * (P['T'][i] - P['TSTEP']) + PHI) for i in range(P['COUNTER'])]
    sineFracPlus  = [scale[0] * np.sin(2 * np.pi * P['F'] * (P['T'][i] + P['TSTEP']) + PHI) for i in range(P['COUNTER'])]
    
    # Square Wave Input
    a = 9.117
    squareFrac      = [np.tanh(a * P['T'][i]               ) for i in range(P['COUNTER'])]
    squareFracMinus = [np.tanh(a * (P['T'][i] - P['TSTEP'])) for i in range(P['COUNTER'])]
    squareFracPlus  = [np.tanh(a * (P['T'][i] + P['TSTEP'])) for i in range(P['COUNTER'])]
    
    for j in [i+1 for i in range(P['N_CYC'])]:
        squareFrac      = [squareFrac[i]      * np.tanh(a * (P['T'][i]                - (j-0.5)/P['F'])) * np.tanh(a * (P['T'][i]                - j / P['F'])) for i in range(P['COUNTER'])]
        squareFracMinus = [squareFracMinus[i] * np.tanh(a * ((P['T'][i] - P['TSTEP']) - (j-0.5)/P['F'])) * np.tanh(a * ((P['T'][i] - P['TSTEP']) - j / P['F'])) for i in range(P['COUNTER'])]
        squareFracPlus  = [squareFracPlus[i]  * np.tanh(a * ((P['T'][i] + P['TSTEP']) - (j-0.5)/P['F'])) * np.tanh(a * ((P['T'][i] + P['TSTEP']) - j / P['F'])) for i in range(P['COUNTER'])]
    
#    squareFrac      = [scale[1] * 2. * np.arctan(np.sin(2. * np.pi * P['F'] * P['T'][i]                + PHI) / delta) / np.pi for i in range(P['COUNTER'])]
#    squareFracMinus = [scale[1] * 2. * np.arctan(np.sin(2. * np.pi * P['F'] * (P['T'][i] - P['TSTEP']) + PHI) / delta) / np.pi for i in range(P['COUNTER'])]
#    squareFracPlus  = [scale[1] * 2. * np.arctan(np.sin(2. * np.pi * P['F'] * (P['T'][i] + P['TSTEP']) + PHI) / delta) / np.pi for i in range(P['COUNTER'])]
    
    # Triangle wave
    triangleFrac      = [scale[2] * (1. - 2. * np.arccos((1. - delta) * np.sin(2. * np.pi * P['F'] * P['T'][i]                + PHI)) / np.pi) for i in range(P['COUNTER'])]
    triangleFracMinus = [scale[2] * (1. - 2. * np.arccos((1. - delta) * np.sin(2. * np.pi * P['F'] * (P['T'][i] - P['TSTEP']) + PHI)) / np.pi) for i in range(P['COUNTER'])]
    triangleFracPlus  = [scale[2] * (1. - 2. * np.arccos((1. - delta) * np.sin(2. * np.pi * P['F'] * (P['T'][i] + P['TSTEP']) + PHI)) / np.pi) for i in range(P['COUNTER'])]
    
    # Saw-tooth Wave
    xt  = [0.25 * (2. * P['F'] * P['T'][i]                - 1.) for i in range(P['COUNTER'])]
    xtm = [0.25 * (2. * P['F'] * (P['T'][i] - P['TSTEP']) - 1.) for i in range(P['COUNTER'])]
    xtp = [0.25 * (2. * P['F'] * (P['T'][i] + P['TSTEP']) - 1.) for i in range(P['COUNTER'])]
    
    xs  = [0.5 * P['F'] * P['T'][i]                for i in range(P['COUNTER'])]
    xsm = [0.5 * P['F'] * (P['T'][i] - P['TSTEP']) for i in range(P['COUNTER'])]
    xsp = [0.5 * P['F'] * (P['T'][i] + P['TSTEP']) for i in range(P['COUNTER'])]
    
    trg  = [1. - 2. * np.arccos((1. - delta) * np.sin(2. * np.pi * xt[i] )) / np.pi for i in range(P['COUNTER'])]
    trgM = [1. - 2. * np.arccos((1. - delta) * np.sin(2. * np.pi * xtm[i])) / np.pi for i in range(P['COUNTER'])]
    trgP = [1. - 2. * np.arccos((1. - delta) * np.sin(2. * np.pi * xtp[i])) / np.pi for i in range(P['COUNTER'])]
    
    sqr  = [2. * np.arctan(np.sin(2. * np.pi * xs[i] ) / delta) / np.pi for i in range(P['COUNTER'])]
    sqrM = [2. * np.arctan(np.sin(2. * np.pi * xsm[i]) / delta) / np.pi for i in range(P['COUNTER'])]
    sqrP = [2. * np.arctan(np.sin(2. * np.pi * xsp[i]) / delta) / np.pi for i in range(P['COUNTER'])]
    
    sawFrac      = [scale[3] * trg[i]  * sqr[i]  for i in range(P['COUNTER'])]
    sawFracMinus = [scale[3] * trgM[i] * sqrM[i] for i in range(P['COUNTER'])]
    sawFracPlus  = [scale[3] * trgP[i] * sqrP[i] for i in range(P['COUNTER'])]

    # Form composite signal
    signal      = [P['SIG_WEIGHT'][0] * sineFrac[i]      + P['SIG_WEIGHT'][1] * squareFrac[i]      + P['SIG_WEIGHT'][2] * triangleFrac[i]      + P['SIG_WEIGHT'][3] * sawFrac[i]      for i in range(P['COUNTER'])]
    signalMinus = [P['SIG_WEIGHT'][0] * sineFracMinus[i] + P['SIG_WEIGHT'][1] * squareFracMinus[i] + P['SIG_WEIGHT'][2] * triangleFracMinus[i] + P['SIG_WEIGHT'][3] * sawFracMinus[i] for i in range(P['COUNTER'])]
    signalPlus  = [P['SIG_WEIGHT'][0] * sineFracPlus[i]  + P['SIG_WEIGHT'][1] * squareFracPlus[i]  + P['SIG_WEIGHT'][2] * triangleFracPlus[i]  + P['SIG_WEIGHT'][3] * sawFracPlus[i]  for i in range(P['COUNTER'])]
...
def vel_multi_kinematics(P, sig):
    ...
    signal_minus = sig[1]
    signal_plus  = sig[2]
    vel       = [0. for i in range(P['COUNTER'])]
    vel_minus = [0. for i in range(P['COUNTER'])]
    vel_plus  = [0. for i in range(P['COUNTER'])]
    ...
    vel       = [vel[i]       / DEL_T for i in range(P['COUNTER'])]
    vel_minus = [vel_minus[i] / DEL_T for i in range(P['COUNTER'])]
    vel_plus  = [vel_plus[i]  / DEL_T for i in range(P['COUNTER'])]
...
def accel_multi_kinematics(P, sig):
    ...
    signal_minus = sig[1]
    signal_plus  = sig[2]
    accell       = [0. for i in range(P['COUNTER'])]
    accell_minus = [0. for i in range(P['COUNTER'])]
    accell_plus  = [0. for i in range(P['COUNTER'])]
    ...
    accell       = [accell[i]       / DEL_T**2 for i in range(P['COUNTER'])]
    accell_minus = [accell_minus[i] / DEL_T**2 for i in range(P['COUNTER'])]
    accell_plus  = [accell_plus[i]  / DEL_T**2 for i in range(P['COUNTER'])]
